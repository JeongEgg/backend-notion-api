...
import com.example.backend_notion_api.config.AwsS3Config;
import com.example.backend_notion_api.exception.customexception.FileDownloadException;
import com.example.backend_notion_api.exception.customexception.FileUploadException;
...
public class S3Service {
    ...
    public void uploadFileWithKeyname(MultipartFile multipartFile, String keyName){
        try {
            // 오브젝트 키는 인코딩하지 않고 원본 파일 이름을 그대로 사용
            PutObjectRequest putObjectRequest = PutObjectRequest.builder()
                    .bucket(bucketName)
            ...
            // 업로드 중 IOException이 발생할 경우 FileUploadException 발생
            throw new FileUploadException("파일 업로드 중 오류 발생: " + e.getMessage());
        } catch (SdkException e) {
            // AWS SDK 관련 예외 처리
            throw new FileUploadException("AWS S3 업로드 중 오류 발생: " + e.getMessage());
        }
    }
    ...
    public String downloadFileAsURL(String keyName) {
        // S3Presigner 생성
